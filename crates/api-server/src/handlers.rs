...
pub async fn list_containers() -> impl Responder {
    ...
    match ContainerManager::list().await {
        ...
        Err(e) => {
            ...
            // Map LXC command failures to 503 Service Unavailable
            if matches!(e, ContainerError::LxcCommandFailed(_)) {
                HttpResponse::ServiceUnavailable().json(serde_json::json!({
                    "error": "LXC service is unavailable"
                }))
            } else {
...
pub async fn create_storage_pool(req: web::Json<CreateStoragePoolRequest>) -> impl Responder {
    ...
    match result {
        ...
        Err(e) => {
            ...
            // Service unavailable for IO errors or command failures
            if matches!(e, StorageError::Io(_) | StorageError::CommandFailed(_)) {
                HttpResponse::ServiceUnavailable().json(serde_json::json!({
...
pub async fn list_bridges() -> impl Responder {
    ...
    match BridgeManager::list().await {
        ...
        Err(e) => {
            ...
            if matches!(e, NetworkError::CommandFailed(_) | NetworkError::Io(_)) {
                HttpResponse::ServiceUnavailable().json(serde_json::json!({
